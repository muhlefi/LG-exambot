                ...
                <div class="group grid gap-4 p-6 transition-all hover:bg-white/90 md:grid-cols-[1fr_auto]">
                    ...
                    <div class="flex items-center gap-6">
                        ...
                        <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <form id="deleteSession{{ $session->id }}" method="POST" action="{{ route('sessions.destroy', $session) }}">
                                @csrf
                                @method('DELETE')
                                <button type="button" onclick="confirmDelete('deleteSession{{ $session->id }}', 'Sesi ini dan seluruh soal di dalamnya akan dihapus permanen!')" class="rounded-full border border-clay/20 bg-white p-2 text-clay hover:bg-clay hover:text-white">
